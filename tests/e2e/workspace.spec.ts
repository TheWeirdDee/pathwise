...
test('intent → score → fixture receipt → recomputed proof', async ({ page, request }) => {
  ...
  await page.locator('select#pocket').selectOption('USDM');
  ...
  await page.locator('textarea, input[type="text"]').first().fill('do something');
...
test('all routes render; campaign records all attempts', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /Run fixture campaign|Rerun fixture campaign/i }).click();
