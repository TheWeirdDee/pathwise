...
async function gotoHydrated(page: any, path: string) {
...
test('all routes render; campaign records all attempts',async({page})=>{
  for(const route of ['/wallets','/receipts','/proof','/policy','/campaign']){await gotoHydrated(page, route);await expect(page.locator('h1')).toBeVisible();}
...
test('emergency stop persists and server rejects new runs',async({page,request})=>{
  ...
  const result=await request.post('/api/receipts',{data:{text:'Buy 200 USDT of SOL'}});expect(result.status()).toBe(409);
  await page.reload();
  await expect(page.locator('.app-shell[data-hydrated="true"]')).toBeVisible();
  await expect(page.getByRole('button',{name:'Find best path'})).toBeDisabled();
